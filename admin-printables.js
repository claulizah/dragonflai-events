...
export default async (request, context) => {
  ...
  try {
    ...
    if (action === 'delete' && request.method === 'POST') {
      ...
      // No se puede borrar de verdad un diseño que ya tiene compras —
      // rompería el historial (no se podría ver qué compró alguien, ni
      // reenviar su archivo si lo pide). Ahí solo cabe desactivar.
      const purchasesRes = await fetch(
        `${SUPABASE_URL}/rest/v1/printable_purchases?design_id=eq.${encodeURIComponent(id)}&select=id&limit=1`,
        { headers: serviceHeaders(serviceKey) }
      );
      if (!purchasesRes.ok) return json({ error: 'Error verificando compras: ' + await purchasesRes.text() }, 500);
      const purchasesArr = await purchasesRes.json();
      if (purchasesArr.length > 0) {
        return json({ error: 'Este diseño ya tiene compras asociadas — no se puede eliminar. Usa Desactivar en su lugar.' }, 409);
      }

      const delResp = await fetch(`${SUPABASE_URL}/rest/v1/printable_designs?id=eq.${encodeURIComponent(id)}`, {
        method: 'DELETE',
        headers: serviceHeaders(serviceKey)
      });
      if (!delResp.ok) return json({ error: 'Error eliminando: ' + await delResp.text() }, 500);
      return json({ success: true });
    }
